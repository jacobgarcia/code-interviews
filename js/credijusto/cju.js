...
isPreposition = word => {
  if (word.length === 6 && foo.includes(word[word.length - 1]) && !word.includes('u')) return true
  return false
}
...
isVerb = word => {
  ...
  if (word.length >= 6 && !foo.includes(word[word.length - 1]) && !foo.includes(word[0])) {
    response.subjunctive = true
    response.verb = true
  } else if (word.length >= 6 && !foo.includes(word[word.length - 1])) response.verb = true
  return response
}
...
main = text => {
  prototype.map.call(lex, (char, index) => {
    lexicalMap[char] = index + 1
  })
  ...
  console.log(`3) There are ${response.verbs.subjunctives} subjunctive verbs in the text`)
  console.log(`4) Vocabulary list: ${vocabulary.sort(customSort()).join(' ')}`)
  console.log(`5) There are ${response.numbers.pretties} distinct pretty numbers in the text`)
} 
